...
void stu_insert(ppst stu_h,ppst stu_t)//插入
{
    long i=-1;
    int c=0;
    printf("请输入需要添加学生的位置(如果直接插入在末尾则输入-1):");
    scanf("%ld",&i);
    *stu_t=*stu_h;
    pst stu=(pst)calloc(1,sizeof(st));
    printf("请输入学生姓名:");scanf("%s",stu->name);
...
void stu_num_order(ppst stu_h,ppst stu_t)//按学号排序
{
    if(*stu_h==NULL) {printf("学生档案为空！");return;}
  ...
  main()
{
    ...
    do{
    printf("管理员具有对档案修改和删除权限\n");
    printf("管理员初始密码为:0000  \n是否重置管理员密码？(是/y 否/n)\n");
    scanf("%s",&If);
    }while(If!='y'&&If!='n'&&If!='Y'&&If!='N');
    ...
    if(IF==0)  {stu_create(&stu_h,&stu_t);stu_print(stu_h);}
    else if(IF==1) {stu_read(&stu_h,&stu_t);stu_print(stu_h);}
modify:
    {
    do
    {
    ...
    scanf("%d",&IF);
    system("cls");
    }while(IF!=0&&IF!=1&&IF!=2&&IF!=3&&IF!=4&&IF!=5&&IF!=6&&IF!=7);
    switch(IF)
    {
    ...
    if(If=='y'||If=='Y') {goto modify;system("cls");}
    }
    End:
    {
    stu_save(stu_h);
